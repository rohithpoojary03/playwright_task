...
test('Verify Bearer Token Authentication', async ({ request }) => {
  const bearerToken = "(generate your own token from github and use here)"; // Replace with a real token
...
test('Bearer Token Auth', async ({ request }) => {
  const token = '(generate your own token from github and use here)'; // Replace with a real token
...
test('API Key Auth - Header', async ({ request }) => {
  const response = await request.get('https://api.weatherapi.com/v1/current.json', {
    params: { q: 'India', key: '<YOUR_API_KEY>' },
  });
  expect(response.status()).toBe(200);
  const data = await response.json();
  console.log(data);
